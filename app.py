...
def check_password():
    def password_entered():
        if st.session_state["password"] == "tn0000":
            ...
            st.sidebar.error("❌ パスワードが違います。アクセス権限がありません。")
...
# 認証が成功した場合のみ、以下の全システム・UIモジュールをメモリに展開
if check_password():
    if os.path.exists("logo.png"): 
    ...
    crack_type = st.sidebar.selectbox("⑥ 目視での主たる劣化症状",
